...
class BrotliCompressorTest {
	...
	@Test
	void compressCreatesBrotliFile() throws Exception {
		Path source = this.tempDir.resolve("test.js");
		Files.writeString(source, "console.log('hello world');");

		Path target = this.tempDir.resolve("test.js.br");
		BrotliCompressor compressor = new BrotliCompressor(11);
	...
	void compressedContentCanBeDecompressed() throws Exception {
		...
		Path target = this.tempDir.resolve("style.css.br");
		BrotliCompressor compressor = new BrotliCompressor(11);
		compressor.compress(source, target);

		try (BrotliInputStream bis = new BrotliInputStream(Files.newInputStream(target))) {
			String decompressed = new String(bis.readAllBytes());
			assertThat(decompressed).isEqualTo(originalContent);
		}
	}

	@Test
	void compressWithLowQuality() throws Exception {
		Path source = this.tempDir.resolve("test.js");
		Files.writeString(source, "console.log('hello world');");

		Path target = this.tempDir.resolve("test.js.br");
		BrotliCompressor compressor = new BrotliCompressor(0);
		long compressedSize = compressor.compress(source, target);
	...
	void suffixAndLabel() {
		BrotliCompressor compressor = new BrotliCompressor(11);
		assertThat(compressor.suffix()).isEqualTo(".br");
		assertThat(compressor.label()).isEqualTo("br");
	}
